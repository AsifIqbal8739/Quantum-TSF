...
def generate_circuit(
    ...
    parameters_encoding = ParameterVector('e', length=num_parameters_encoding)

    # variational parameters (`depth+1` instances)
    num_parameters_variational = 2 * num_qubits * (depth + 1)
    parameters_variational = ParameterVector('v', length=num_parameters_variational)

    # optional input scaling
    parameters_scaling = None
    if input_scaling:
        num_parameters_scaling = 2 * num_qubits * depth
        parameters_scaling = ParameterVector('s', length=num_parameters_scaling)
...
# Here the Data Reuploading is done at the start of the layer instead of the end
def generate_circuit_2(
        num_qubits: int = 4,
        depth: int = 1,
        entanglement_structure: str = 'nn',
        input_scaling: bool = False,
